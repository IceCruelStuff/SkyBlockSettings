...
class Main extends BasicPlugin implements CommandExecutor, Listener
{
    ...
    public function onEnable()
    {
        ...
        $cfg = $this->modConfig(__NAMESPACE__, ["max-players" => ["MaxPlayerMgr", false], "protect" => ["WpProtectMgr", true], "border" => ["WpBordersMgr", true], "pvp" => ["WpPvpMgr", true], "motd" => ["WpMotdMgr", false], "no-explode" => ["NoExplodeMgr", false], "unbreakable" => ["Unbreakable", false], "bancmds" => ["BanCmd", false], "banitem" => ["BanItem", true], "gamemode" => ["GmMgr", false], "gm-save-inv" => ["GmSaveInv", false]], ["version" => $this->getDescription()->getVersion(), "motd" => WpMotdMgr::defaults()], mc::_("/%s [世界] %s %s"));
    ...
    public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args) : bool
    {
        ...
        if ($world === null) {
            $sender->sendMessage(mc::_("[WP] 一定要指定一个世界"));
            return false;
        }
    ...
    public function isAuth($c, $world)
    {
        ...
        $c->sendMessage(mc::_("[WP] 你不被允许完成这个动作"));
        return false;
    }
