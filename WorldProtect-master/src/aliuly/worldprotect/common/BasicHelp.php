    ...
    public function onSCommand(CommandSender $c, Command $cc, $scmd, $data, array $args)
    {
        ...
        if (count($args)) {
            ...
            foreach ($args as $i) {
                ...
                $txt[] = TextFormat::YELLOW . mc::_("Help: ") . TextFormat::WHITE . "/" . $cc->getName() . " {$i}";
                if ($cm->getHelpMsg($i) !== null) {
                    $txt[] = TextFormat::YELLOW . mc::_("Description: ") . TextFormat::WHITE . $cm->getHelpMsg($i);
                }
                if ($cm->getUsage($i) !== null) {
                    $txt[] = TextFormat::YELLOW . mc::_("Usage: ") . TextFormat::WHITE . sprintf($this->fmt, $cc->getName(), $i, $cm->getUsage($i));
                }
            }
