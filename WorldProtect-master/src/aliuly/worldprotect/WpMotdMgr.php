...
class WpMotdMgr extends BaseWp implements Listener, CommandExecutor
{
    ...
    public function __construct(Plugin $plugin, $cfg)
    {
        ...
        $this->enableSCmd("motd", ["usage" => mc::_("[text]"), "help" => mc::_("编辑世界的MOTD"), "permission" => "wp.cmd.wpmotd"]);
        $this->enableCmd("motd", ["description" => mc::_("显示世界的MOTD"), "usage" => "/motd [world]", "permission" => "wp.motd"]);
    }
    ...
    public function onSCommand(CommandSender $c, Command $cc, $scmd, $world, array $args)
    {
        ...
        if (count($args) == 0) {
            ...
            $c->sendMessage(mc::_("[WP] %1% 世界的MOTD已被移除", $world));
            return true;
        }
        $this->owner->setCfg($world, "motd", implode(" ", $args));
        $c->sendMessage(mc::_("[WP] %1% 世界的MOTD已更新", $world));
        return true;
    }
    private function showMotd($c, $world)
    {
        if (!$c->hasPermission("wp.motd")) {
            return;
        }
