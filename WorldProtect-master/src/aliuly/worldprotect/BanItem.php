...
class BanItem extends BaseWp implements Listener
{
    public function __construct(Plugin $plugin)
    {
        ...
        $this->enableSCmd("banitem", ["usage" => mc::_("[id] ..."), "help" => mc::_("禁用一個物品"), "permission" => "wp.cmd.banitem"]);
        $this->enableSCmd("unbanitem", ["usage" => mc::_("[id] ..."), "help" => mc::_("解禁一個物品"), "permission" => "wp.cmd.banitem"]);
    }
    public function onSCommand(CommandSender $c, Command $cc, $scmd, $world, array $args)
    {
        ...
        if (count($args) == 0) {
            ...
            if (count($ids) == 0) {
                $c->sendMessage(mc::_("[WP] %1% 世界沒有被禁用物品", $world));
            } else {
    ...
    public function onInteract(PlayerInteractEvent $ev)
    {
        ...
        $pl->sendMessage(mc::_("§c!!! 該物品在本世界為禁用狀態 !!!"));
        $ev->setCancelled();
    }
