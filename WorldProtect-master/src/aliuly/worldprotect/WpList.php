...
class WpList extends BaseWp
{
    public function __construct($owner)
    {
        ...
        $this->enableSCmd("ls", ["usage" => mc::_("[world]"), "help" => mc::_("顯示世界中的保護事項"), "permission" => "wp.cmd.info", "aliases" => ["info"]]);
    }
    public function onSCommand(CommandSender $c, Command $cc, $scmd, $world, array $args)
    {
        $pageNumber = $this->getPageNumber($args);
        if (count($args) == 1) {
            return $this->wpDetails($c, $args[0], $pageNumber);
        }
        if (count($args) == 0) {
            return $this->wpList($c, $pageNumber);
        }
        return false;
    }
    private function wpDetails(CommandSender $c, $world, $pageNumber)
    {
        if (!$this->owner->getServer()->isLevelGenerated($world)) {
            $c->sendMessage(mc::_("%1% 世界並不存在", $world));
            return;
        }
        $f = $this->owner->getServer()->getDataPath() . "worlds/{$world}/wpcfg.yml";
        if (!is_file($f)) {
            $c->sendMessage(mc::_("%1% 世界不被保護", $world));
            return;
        }
        $wcfg = (new Config($f, Config::YAML))->getAll();
        $txt = [mc::_("本世界 - 信息管理", $world)];
        if (isset($wcfg["protect"])) {
            $txt[] = TextFormat::AQUA . mc::_("保護:  ") . TextFormat::WHITE . $wcfg["protect"];
        }
        if (isset($wcfg["max-players"])) {
            $txt[] = TextFormat::AQUA . mc::_("玩家限制:  ") . TextFormat::WHITE . $wcfg["max-players"];
        }
        if (isset($wcfg["gamemode"])) {
            $txt[] = TextFormat::AQUA . mc::_("遊戲模式:  ") . TextFormat::WHITE . MPMU::gamemodeStr($wcfg["gamemode"]);
        }
        if (isset($wcfg["pvp"])) {
            if ($wcfg["pvp"] === true) {
        ...
        if (isset($wcfg["no-explode"])) {
            if ($wcfg["no-explode"] === "off") {
                $txt[] = TextFormat::AQUA . mc::_("無爆炸: ") . TextFormat::RED . mc::_("off");
            } elseif ($wcfg["no-explode"] === "world") {
                $txt[] = TextFormat::AQUA . mc::_("無爆炸: ") . TextFormat::GREEN . mc::_("world");
            } else {
                $txt[] = TextFormat::AQUA . mc::_("無爆炸: ") . TextFormat::YELLOW . mc::_("spawn");
            }
        }
        if (isset($wcfg["border"])) {
            $txt[] = TextFormat::AQUA . mc::_("邊界: ") . TextFormat::WHITE . implode(",", $wcfg["border"]);
        }
        if (isset($wcfg["auth"])) {
            $txt[] = TextFormat::AQUA . mc::_("授權列表(%1%): ", count($wcfg["auth"])) . TextFormat::WHITE . implode(",", $wcfg["auth"]);
        }
        if (isset($wcfg["unbreakable"])) {
            $txt[] = TextFormat::AQUA . mc::_("不能破壞方塊(%1%): ", count($wcfg["unbreakable"])) . TextFormat::WHITE . implode(",", $wcfg["unbreakable"]);
        }
        if (isset($wcfg["bancmds"])) {
            $txt[] = TextFormat::AQUA . mc::_("禁用指令(%1%): ", count($wcfg["bancmds"])) . TextFormat::WHITE . implode(",", $wcfg["bancmds"]);
        }
        if (isset($wcfg["banitem"])) {
            $txt[] = TextFormat::AQUA . mc::_("禁用(%1%): ", count($wcfg["banitem"])) . TextFormat::WHITE . implode(",", $wcfg["banitem"]);
        }
        if (isset($wcfg["motd"])) {
            $txt[] = mc::_("MOTD:");
    ...
    private function wpList(CommandSender $c, $pageNumber)
    {
        ...
        if (!is_dir($dir)) {
            $c->sendMessage(mc::_("[WP] 缺少路徑 %1%", $dir));
            return true;
        }
        $txt = [];
        $dh = opendir($dir);
        ...
        if (!count((array)$ln)) {
            $c->sendMessage(mc::_("沒什麼可報告的"));
            return true;
        }
        array_unshift($txt, mc::_("世界: %1%", $cnt));
        return $this->paginateText($c, $pageNumber, $txt);
    }
}